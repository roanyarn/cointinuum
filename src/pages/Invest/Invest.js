...
import RoadmapMobile from '../../images/invest/roadmap-mobile.png';
...
const Invest = () => {
	...
	const showInvestImage = (item) => {
		...
		setInvestCard({ id, name, image, title, bulletPoints, text, text2, button});
		if (!isOpen) {
			setShowCard(true);
			setIsOpen(true);
			scrollDown();
		...
		<div className={darkMode ? 'invest invest__dark' : 'invest'}>
			...
			{roadmap && (
				<img
					...
					src={mobileView ? RoadmapMobile : Roadmap}
					alt="roadmap"
				/>
			)}
		</div>
